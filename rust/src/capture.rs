...
impl Default for ReplyExtractor {
    fn default() -> Self {
        Self {
            ...
            ignore_keywords: vec![
                "请", "轮到", "输入", "选项", "worker-",
                "Welcome", "What's", "bypass", "Tips for",
                "Added", "Status line", "/release-notes",
                "Cogitated for", "Worked for", "Churned for",
                "Bypassed for", "@",
            ],
            min_reply_length: 4,
        }
...
impl ReplyExtractor {
    pub fn with_options(
        ...
        Self { ignore_prefixes, ignore_keywords, min_reply_length }
    }
    ...
    pub fn extract(&self, lines: &[&str]) -> String {
        ...
        for &line in lines.iter().rev() {
            ...
            if self.is_noise(trimmed) { continue; }
            if trimmed.len() < self.min_reply_length && !trimmed.starts_with('*') { continue; }
...
fn extract_number_from_text(text: &str) -> Option<i32> {
    ...
    let caps: Vec<i32> = re.captures_iter(text)
